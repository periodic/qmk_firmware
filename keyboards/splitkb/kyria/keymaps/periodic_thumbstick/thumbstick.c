...
void thumbstick_process(void) {
    if (timer_elapsed(thumbstickTimer) > THUMBSTICK_TIMEOUT) {
        ...
        thumbstick_state.vector.y = -thumbstick_get_component(THUMBSTICK_PIN_Y);
#endif
#ifdef THUMBSTICK_ROTATION
        thumbstick_vector_t new_vector;
        new_vector.x =
            cos(THUMBSTICK_ROTATION) * (float)thumbstick_state.vector.x -
            sin(THUMBSTICK_ROTATION) * (float)thumbstick_state.vector.y;
        new_vector.y =
            sin(THUMBSTICK_ROTATION) * (float)thumbstick_state.vector.x +
            cos(THUMBSTICK_ROTATION) * (float)thumbstick_state.vector.y;
        thumbstick_state.vector = new_vector;
#endif
        ...
        switch (thumbstick_state.config.mode) {
            ...
            case THUMBSTICK_MODE_ARROWS:
                ...
                if (timer_elapsed(thumbstickLogTimer) > 100) {
                    thumbstickLogTimer = timer_read();
                    uprintf(
                        "Up %d; Down %d; Left: %d; Right %d; Vec (%d, %d);\n",
                        thumbstick_state.direction.up,
                        thumbstick_state.direction.down,
                        thumbstick_state.direction.left,
                        thumbstick_state.direction.right,
                        thumbstick_state.vector.x,
                        thumbstick_state.vector.y
                    );
                }
#endif
                break;
            case THUMBSTICK_MODE_NUMBERS:
                update_keycode_status(KC_1, thumbstick_state.lastDirection.up, thumbstick_state.direction.up);
                update_keycode_status(KC_2, thumbstick_state.lastDirection.left, thumbstick_state.direction.left);
                update_keycode_status(KC_3, thumbstick_state.lastDirection.right, thumbstick_state.direction.right);
                update_keycode_status(KC_4, thumbstick_state.lastDirection.down, thumbstick_state.direction.down);
                thumbstick_state.lastDirection = thumbstick_state.direction;
#ifdef THUMBSTICK_DEBUG
                if (timer_elapsed(thumbstickLogTimer) > 100) {
                    thumbstickLogTimer = timer_read();
