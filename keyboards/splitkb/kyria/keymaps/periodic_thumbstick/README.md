# Periodic's keymap for Kyria Rev2

Based on Gotham's keymap. The following is their readme:
...
- **THUMBSTICK_PIN_X/Y (mandatory):** The QMK pins to use for the respective axis. The values are from the [QMK's ADC driver](https://docs.qmk.fm/#/adc_driver). I used F0 and F1, for example.
...
- **THUMBSTICK_MODE <mode>:** One of THUMBSTICK_MODE_MOUSE, THUMBSTICK_MODE_ARROWS and THUMBSTICK_MODE_SCROLL. This is just the default mode, it can be changed by calling `void thumbstick_mode_cycle(bool reverse)` within code.
